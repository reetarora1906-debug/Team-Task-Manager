...
const Team = () => {
  ...
  // Sections
  const teamMembers = filtered.filter(u => u.isOnTeam || u.role === 'Admin');
  const availableUsers = filtered.filter(u => !u.isOnTeam && u.role !== 'Admin');
  ...
  const UserCard = ({ u }) => (
    <div className="bg-white rounded-[var(--radius-lg)] border border-[var(--color-outline-variant)]/15 p-5 hover:shadow-[var(--shadow-md)] ">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {user?.role === 'Admin' && u._id !== user._id && (
            <select value={u.role} onChange={e => handleRoleChange(u._id, e.target.value)} className="text-xs px-2 py-1 rounded-full border border-[var(--color-outline-variant)]/30 bg-white outline-none cursor-pointer font-medium">
